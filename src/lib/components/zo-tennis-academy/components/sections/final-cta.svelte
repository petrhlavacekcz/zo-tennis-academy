<script lang="ts">
	import { Button } from "$lib/components/ui/button";
	import ArrowRight from "@lucide/svelte/icons/arrow-right";
	import Phone from "@lucide/svelte/icons/phone";
	import { localizeHref } from "$lib/utils/localize";
	import * as m from "$lib/paraglide/messages";
</script>

<section class="py-16 md:py-24 bg-primary text-primary-foreground relative overflow-hidden">
	<!-- Background Pattern -->
	<div class="absolute inset-0 opacity-10">
		<div class="w-full h-full" style="background-image: radial-gradient(circle at 25% 25%, white 2px, transparent 2px), radial-gradient(circle at 75% 75%, white 2px, transparent 2px); background-size: 50px 50px;"></div>
	</div>

	<div class="mx-auto max-w-[1320px] px-6 md:px-12 relative z-10">
		<h2 class="text-section-heading mb-6">{m["final_cta.heading"]()}</h2>

		<p class="text-lg mb-8 max-w-2xl opacity-90 leading-relaxed">{m["final_cta.description"]()}</p>

		<div class="flex flex-col sm:flex-row gap-4 sm:justify-start items-stretch sm:items-center">
			<Button href={localizeHref("/contact")} variant="cta-inverse" size="xl" class="group">
				<Phone size={20} />
				{m["final_cta.cta_primary"]()}
				<ArrowRight size={20} class="group-hover:translate-x-1 transition-transform" />
			</Button>

			<Button href={localizeHref("/programs")} variant="outline-inverse" size="xl">
				{m["final_cta.cta_secondary"]()}
			</Button>
		</div>

		<!-- Contact Info -->
		<div class="mt-12 pt-8 border-t border-white/20">
			<p class="text-base opacity-90">
				{m["final_cta.contact_text"]()}
				<a href="tel:+420603441399" class="font-semibold hover:underline">{m["final_cta.phone"]()}</a>
			</p>
		</div>
	</div>
</section>
